...
public class TbGpsGeofencingActionNodeConfiguration extends TbGpsGeofencingFilterNodeConfiguration {
    ...
    /**
     * 配置字段：来自规则节点 JSON 的 `minInsideDuration` 配置项，控制与本类处理流程相关的运行时值。
     */
    private int minInsideDuration;
    /**
     * 配置字段：来自规则节点 JSON 的 `minOutsideDuration` 配置项，控制时间戳。
     */
    private int minOutsideDuration;

    /**
     * 配置字段：来自规则节点 JSON 的 `minInsideDurationTimeUnit` 配置项，控制距离单位。
     */
    private String minInsideDurationTimeUnit;
    /**
     * 配置字段：来自规则节点 JSON 的 `minOutsideDurationTimeUnit` 配置项，控制距离单位。
     */
    private String minOutsideDurationTimeUnit;

    /**
     * 配置字段：来自规则节点 JSON 的 `reportPresenceStatusOnEachMessage` 配置项，控制与本类处理流程相关的运行时值。
     */
