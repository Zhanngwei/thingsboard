...
@Data
/**
 * 中文说明：`TbJsonPathNodeConfiguration` 是JSON路径节点配置对象，用于承载规则节点 JSON 中的配置项和默认值。
...
public class TbJsonPathNodeConfiguration implements NodeConfiguration<TbJsonPathNodeConfiguration> {
    ...
    /**
     * 配置辅助常量：`DEFAULT_JSON_PATH` 不从规则节点 JSON 直接读取，用于为相关配置提供默认值、模板或兼容字段名。
     */
    static final String DEFAULT_JSON_PATH = "$";
    /**
     * 配置字段：来自规则节点 JSON 的 `jsonPath` 配置项，控制JavaScript 脚本文本。
     */
    ...
    /*
     * 本类总结：`TbJsonPathNodeConfiguration` 负责转换消息体、元数据、发起实体或拆分/包装规则链消息；作为节点时遵循 Rule Engine 的输入、输出、失败和生命周期约定，作为配置或 helper 时仅承载对应数据和辅助逻辑。
