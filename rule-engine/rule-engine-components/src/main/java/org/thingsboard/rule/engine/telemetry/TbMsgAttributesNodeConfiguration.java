...
@Data
/**
 * 中文说明：`TbMsgAttributesNodeConfiguration` 是消息属性节点配置对象，用于承载规则节点 JSON 中的配置项和默认值。
...
public class TbMsgAttributesNodeConfiguration implements NodeConfiguration<TbMsgAttributesNodeConfiguration> {
    ...
    /**
     * 配置字段：来自规则节点 JSON 的 `scope` 配置项，控制属性作用域。
     */
    private String scope;

    /**
     * 配置字段：来自规则节点 JSON 的 `notifyDevice` 配置项，控制与本类处理流程相关的运行时值。
     */
    private boolean notifyDevice;
    /**
     * 配置字段：来自规则节点 JSON 的 `sendAttributesUpdatedNotification` 配置项，控制与本类处理流程相关的运行时值。
     */
    private boolean sendAttributesUpdatedNotification;
    /**
     * 配置字段：来自规则节点 JSON 的 `updateAttributesOnlyOnValueChange` 配置项，控制计算值或最近值。
     */
    ...
    /*
     * 本类总结：`TbMsgAttributesNodeConfiguration` 负责保存、删除或通知属性与时间序列遥测数据；作为节点时遵循 Rule Engine 的输入、输出、失败和生命周期约定，作为配置或 helper 时仅承载对应数据和辅助逻辑。
