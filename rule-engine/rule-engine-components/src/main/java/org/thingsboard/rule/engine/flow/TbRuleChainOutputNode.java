...
/**
 * 中文说明：`TbRuleChainOutputNode` 是规则链输出节点规则节点，用于控制规则链输入输出、确认和检查点流转。
...
public class TbRuleChainOutputNode implements TbNode {
    ...
    /*
     * 本类总结：`TbRuleChainOutputNode` 负责控制规则链输入输出、确认和检查点流转；作为节点时遵循 Rule Engine 的输入、输出、失败和生命周期约定，作为配置或 helper 时仅承载对应数据和辅助逻辑。
