...
/**
 * 中文说明：`TbFetchDeviceCredentialsNodeConfiguration` 是获取设备凭据节点配置对象，用于承载规则节点 JSON 中的配置项和默认值。
...
public class TbFetchDeviceCredentialsNodeConfiguration extends TbAbstractFetchToNodeConfiguration implements NodeConfiguration<TbFetchDeviceCredentialsNodeConfiguration> {
    ...
    /*
     * 本类总结：`TbFetchDeviceCredentialsNodeConfiguration` 负责读取、补充或映射消息元数据、实体字段、属性和遥测上下文信息；作为节点时遵循 Rule Engine 的输入、输出、失败和生命周期约定，作为配置或 helper 时仅承载对应数据和辅助逻辑。
