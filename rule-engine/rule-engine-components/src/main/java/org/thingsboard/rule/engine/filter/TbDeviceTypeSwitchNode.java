...
/**
 * 中文说明：`TbDeviceTypeSwitchNode` 是设备类型切换节点规则节点，用于根据消息类型、实体类型、关系、脚本或告警状态判断消息路由。
...
public class TbDeviceTypeSwitchNode extends TbAbstractTypeSwitchNode {
    ...
    @Override
    /**
     * 方法说明：读取配置、消息字段、实体字段或服务返回值，供 `TbDeviceTypeSwitchNode` 的规则节点处理或辅助流程调用。
    ...
    /*
     * 本类总结：`TbDeviceTypeSwitchNode` 负责根据消息类型、实体类型、关系、脚本或告警状态判断消息路由；作为节点时遵循 Rule Engine 的输入、输出、失败和生命周期约定，作为配置或 helper 时仅承载对应数据和辅助逻辑。
