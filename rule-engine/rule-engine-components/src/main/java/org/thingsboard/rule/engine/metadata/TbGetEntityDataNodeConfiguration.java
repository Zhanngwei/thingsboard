...
/**
 * 中文说明：`TbGetEntityDataNodeConfiguration` 是获取实体数据节点配置对象，用于承载规则节点 JSON 中的配置项和默认值。
...
public class TbGetEntityDataNodeConfiguration extends TbGetMappedDataNodeConfiguration implements NodeConfiguration<TbGetEntityDataNodeConfiguration> {
    ...
    /**
     * 配置字段：来自规则节点 JSON 的 `dataToFetch` 配置项，控制消息体数据。
     */
    ...
    /*
     * 本类总结：`TbGetEntityDataNodeConfiguration` 负责读取、补充或映射消息元数据、实体字段、属性和遥测上下文信息；作为节点时遵循 Rule Engine 的输入、输出、失败和生命周期约定，作为配置或 helper 时仅承载对应数据和辅助逻辑。
