...
/**
 * 中文说明：
 * 1. `TbUnassignFromCustomerNodeConfiguration` 是 ThingsBoard Rule Engine Components 中描述客户行为的配置类型。
 * 2. 它集中保存该组件启动或运行时需要的可配置选项。
 * 3. 字段值决定功能开关、限制条件、地址或处理策略等具体行为。
 * 4. 直接依赖的类型边界包括 `TbAbstractCustomerActionNodeConfiguration`、`NodeConfiguration`。
