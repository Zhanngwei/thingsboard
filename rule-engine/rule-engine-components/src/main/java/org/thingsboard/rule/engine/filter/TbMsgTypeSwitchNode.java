...
/**
 * 中文说明：`TbMsgTypeSwitchNode` 是消息类型切换节点规则节点，用于根据消息类型、实体类型、关系、脚本或告警状态判断消息路由。
...
public class TbMsgTypeSwitchNode implements TbNode {
    ...
    /*
     * 本类总结：`TbMsgTypeSwitchNode` 负责根据消息类型、实体类型、关系、脚本或告警状态判断消息路由；作为节点时遵循 Rule Engine 的输入、输出、失败和生命周期约定，作为配置或 helper 时仅承载对应数据和辅助逻辑。
