...
@Data
/**
 * 中文说明：`TbRuleChainInputNodeConfiguration` 是规则链输入节点配置对象，用于承载规则节点 JSON 中的配置项和默认值。
...
public class TbRuleChainInputNodeConfiguration implements NodeConfiguration<TbRuleChainInputNodeConfiguration> {
    ...
    /**
     * 配置字段：来自规则节点 JSON 的 `ruleChainId` 配置项，控制与本类处理流程相关的运行时值。
     */
    ...
    /*
     * 本类总结：`TbRuleChainInputNodeConfiguration` 负责控制规则链输入输出、确认和检查点流转；作为节点时遵循 Rule Engine 的输入、输出、失败和生命周期约定，作为配置或 helper 时仅承载对应数据和辅助逻辑。
