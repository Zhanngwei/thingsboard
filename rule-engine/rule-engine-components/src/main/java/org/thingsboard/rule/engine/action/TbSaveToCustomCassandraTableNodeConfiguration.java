...
@Data
/**
 * 中文说明：`TbSaveToCustomCassandraTableNodeConfiguration` 是保存到自定义Cassandra表节点配置对象，用于承载规则节点 JSON 中的配置项和默认值。
...
public class TbSaveToCustomCassandraTableNodeConfiguration implements NodeConfiguration<TbSaveToCustomCassandraTableNodeConfiguration> {
    ...
    /**
     * 配置字段：来自规则节点 JSON 的 `tableName` 配置项，控制与本类处理流程相关的运行时值。
     */
    ...
    /*
     * 本类总结：`TbSaveToCustomCassandraTableNodeConfiguration` 负责执行告警、客户归属、关系、设备状态、日志或外部存储等动作；作为节点时遵循 Rule Engine 的输入、输出、失败和生命周期约定，作为配置或 helper 时仅承载对应数据和辅助逻辑。
