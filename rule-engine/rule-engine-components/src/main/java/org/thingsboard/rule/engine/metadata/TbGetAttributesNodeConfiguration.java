...
public class TbGetAttributesNodeConfiguration extends TbAbstractFetchToNodeConfiguration implements NodeConfiguration<TbGetAttributesNodeConfiguration> {
    ...
    /**
     * 配置字段：来自规则节点 JSON 的 `clientAttributeNames` 配置项，控制与本类处理流程相关的运行时值。
     */
    private List<String> clientAttributeNames;
    /**
     * 配置字段：来自规则节点 JSON 的 `sharedAttributeNames` 配置项，控制与本类处理流程相关的运行时值。
     */
    private List<String> sharedAttributeNames;
    /**
     * 配置字段：来自规则节点 JSON 的 `serverAttributeNames` 配置项，控制与本类处理流程相关的运行时值。
     */
    ...
    /**
     * 配置字段：来自规则节点 JSON 的 `tellFailureIfAbsent` 配置项，控制与本类处理流程相关的运行时值。
     */
    private boolean tellFailureIfAbsent;
    /**
     * 配置字段：来自规则节点 JSON 的 `getLatestValueWithTs` 配置项，控制计算值或最近值。
     */
