...
/**
 * 中文说明：`DeduplicationId` 是去重标识枚举，用于限定按配置聚合、去重、延迟和输出消息时可选择的固定值。
...
public enum DeduplicationId {
    ...
    /*
     * 本类总结：`DeduplicationId` 负责按配置聚合、去重、延迟和输出消息；作为节点时遵循 Rule Engine 的输入、输出、失败和生命周期约定，作为配置或 helper 时仅承载对应数据和辅助逻辑。
