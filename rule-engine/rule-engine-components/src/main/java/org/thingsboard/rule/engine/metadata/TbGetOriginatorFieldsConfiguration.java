...
/**
 * 中文说明：`TbGetOriginatorFieldsConfiguration` 是获取发起实体字段配置对象，用于承载规则节点 JSON 中的配置项和默认值。
...
public class TbGetOriginatorFieldsConfiguration extends TbGetMappedDataNodeConfiguration implements NodeConfiguration<TbGetOriginatorFieldsConfiguration> {
    ...
    /**
     * 配置字段：来自规则节点 JSON 的 `ignoreNullStrings` 配置项，控制与本类处理流程相关的运行时值。
     */
    ...
    /*
     * 本类总结：`TbGetOriginatorFieldsConfiguration` 负责读取、补充或映射消息元数据、实体字段、属性和遥测上下文信息；作为节点时遵循 Rule Engine 的输入、输出、失败和生命周期约定，作为配置或 helper 时仅承载对应数据和辅助逻辑。
