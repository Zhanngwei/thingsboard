...
@Data
/**
 * 中文说明：`TbCheckMessageNodeConfiguration` 是检查消息节点配置对象，用于承载规则节点 JSON 中的配置项和默认值。
...
public class TbCheckMessageNodeConfiguration implements NodeConfiguration<TbCheckMessageNodeConfiguration>  {
    ...
    /**
     * 配置字段：来自规则节点 JSON 的 `messageNames` 配置项，控制与本类处理流程相关的运行时值。
     */
    private List<String> messageNames;
    /**
     * 配置字段：来自规则节点 JSON 的 `metadataNames` 配置项，控制消息元数据。
     */
    private List<String> metadataNames;

    /**
     * 配置字段：来自规则节点 JSON 的 `checkAllKeys` 配置项，控制消息体、元数据、属性或遥测中的键名。
     */
    ...
    /*
     * 本类总结：`TbCheckMessageNodeConfiguration` 负责根据消息类型、实体类型、关系、脚本或告警状态判断消息路由；作为节点时遵循 Rule Engine 的输入、输出、失败和生命周期约定，作为配置或 helper 时仅承载对应数据和辅助逻辑。
