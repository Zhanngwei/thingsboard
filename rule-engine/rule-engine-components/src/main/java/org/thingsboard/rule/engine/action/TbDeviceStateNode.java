...
public class TbDeviceStateNode implements TbNode {
    ...
    /**
     * 常量字段：定义 `SUPPORTED_EVENTS`，用于时间戳，本身不触发外部系统调用。
     */
    ...
    /**
     * 常量字段：定义 `DEFAULT_RATE_LIMIT_CONFIG`，用于与本类处理流程相关的运行时值，本身不触发外部系统调用。
     */
    ...
    /**
     * 字段说明：保存 `rateLimitConfig`，表示与本类处理流程相关的运行时值，供本类方法在规则节点处理流程中使用。
     */
    private String rateLimitConfig;
    /**
     * 字段说明：保存 `event`，表示与本类处理流程相关的运行时值，供本类方法在规则节点处理流程中使用。
     */
    ...
    /**
     * 方法说明：把处理结果发送到规则链后续关系，供 `TbDeviceStateNode` 的规则节点处理或辅助流程调用。
    ...
    /**
     * 方法说明：读取配置、消息字段、实体字段或服务返回值，供 `TbDeviceStateNode` 的规则节点处理或辅助流程调用。
     * 调用边界：数据库/缓存：本方法本身不直接访问数据库或缓存，具体实现/调用链可能涉及；Rule Engine/Actor：由规则节点运行时调用或通过 `ctx` 投递、确认、调度消息，通常处于 Actor 调度链路；MQTT：本方法本身不直接发布或订阅 MQTT 消息；事务：本方法本身不直接开启或提交事务。
     */
    ...
    @Override
    /**
     * 方法说明：执行 `onPartitionChangeMsg` 对应的辅助逻辑，供 `TbDeviceStateNode` 的规则节点处理或辅助流程调用。
