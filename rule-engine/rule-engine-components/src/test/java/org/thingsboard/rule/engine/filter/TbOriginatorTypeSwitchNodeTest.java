...
/**
 * 测试目标：验证 {@code TbOriginatorTypeSwitchNodeTest} 覆盖的 过滤/分流节点 行为，重点说明配置、消息和断言路径。
 * 所属生产节点/组件：{@code TbOriginatorTypeSwitchNode}，用于守护对应 Rule Engine 组件的兼容性和边界条件。
...
class TbOriginatorTypeSwitchNodeTest {
    ...
    /** 测试常量字段：{@code RANDOM_UUID} 保存 {@code UUID} 测试数据或依赖，来源：由类加载时构造，生命周期覆盖整个测试类执行过程。 */
    private static final UUID RANDOM_UUID = UUID.randomUUID();

    /** 可变 fixture 字段：{@code node} 保存 {@code TbOriginatorTypeSwitchNode} 测试数据或依赖，来源：通常由 setUp/before/init 或测试体赋值，生命周期随单个测试实例。 */
