...
/**
 * 测试目标：验证 {@code TbJsSwitchNodeTest} 覆盖的 过滤/分流节点 行为，重点说明配置、消息和断言路径。
 * 所属生产节点/组件：{@code TbJsSwitchNode}，用于守护对应 Rule Engine 组件的兼容性和边界条件。
...
public class TbJsSwitchNodeTest {
    ...
    /** 可变 fixture 字段：{@code node} 保存 {@code TbJsSwitchNode} 测试数据或依赖，来源：通常由 setUp/before/init 或测试体赋值，生命周期随单个测试实例。 */
    ...
    /**
     * 测试方法：覆盖 {@code multipleRoutesAreAllowed} 场景，方法名中的 given/when/then 描述输入、触发动作和期望结果。
