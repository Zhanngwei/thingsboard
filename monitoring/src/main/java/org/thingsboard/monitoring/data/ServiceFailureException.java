...
/**
 * 中文说明：
 * 1. 类目的：`ServiceFailureException` 是 ThingsBoard Monitoring 模块 中的健康探测与通知类型，用于对 ThingsBoard 服务、传输协议和 WebSocket/REST 链路执行健康检查、延迟采集与告警通知。
...
public class ServiceFailureException extends RuntimeException {
    ...
    /**
     * 方法说明：
     * 1. 职责：执行 `ServiceFailureException` 对应的健康探测与通知类型流程，完成配置读取、连接管理、协议处理、页面操作、健康探测或测试断言。
...
/*
 * 本类总结：
 * 1. 核心职责：`ServiceFailureException` 在 ThingsBoard Monitoring 模块 中承担健康探测与通知类型职责，核心目的是对 ThingsBoard 服务、传输协议和 WebSocket/REST 链路执行健康检查、延迟采集与告警通知。
