...
/**
 * 中文说明：
 * 1. 类目的：`MonitoredServiceKey` 是 ThingsBoard Monitoring 模块 中的健康探测与通知类型，用于对 ThingsBoard 服务、传输协议和 WebSocket/REST 链路执行健康检查、延迟采集与告警通知。
...
/*
 * 本类总结：
 * 1. 核心职责：`MonitoredServiceKey` 在 ThingsBoard Monitoring 模块 中承担健康探测与通知类型职责，核心目的是对 ThingsBoard 服务、传输协议和 WebSocket/REST 链路执行健康检查、延迟采集与告警通知。
