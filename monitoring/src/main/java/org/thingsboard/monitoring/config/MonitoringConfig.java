...
/**
 * 中文说明：
 * 1. 类目的：`MonitoringConfig` 是 ThingsBoard Monitoring 模块 中的监控配置模型类型，用于承载目标服务、协议探测、设备凭据、阈值和通知配置的数据契约。
...
public interface MonitoringConfig<T extends MonitoringTarget> {
    ...
    /**
     * 方法说明：
     * 1. 职责：执行 `getTargets` 对应的监控配置模型类型流程，完成配置读取、连接管理、协议处理、页面操作、健康探测或测试断言。
...
/*
 * 本类总结：
 * 1. 核心职责：`MonitoringConfig` 在 ThingsBoard Monitoring 模块 中承担监控配置模型类型职责，核心目的是承载目标服务、协议探测、设备凭据、阈值和通知配置的数据契约。
