...
/**
 * 中文说明：
 * 1. 类目的：`TransportMonitoringConfig` 是 ThingsBoard Monitoring 模块 中的监控配置模型类型，用于承载目标服务、协议探测、设备凭据、阈值和通知配置的数据契约。
...
public abstract class TransportMonitoringConfig implements MonitoringConfig<TransportMonitoringTarget> {
    ...
    /**
     * 字段说明：
     * 1. 保存 `targets` 对应的配置、客户端、通道、测试夹具、页面元素、回调或运行期状态。
...
/*
 * 本类总结：
 * 1. 核心职责：`TransportMonitoringConfig` 在 ThingsBoard Monitoring 模块 中承担监控配置模型类型职责，核心目的是承载目标服务、协议探测、设备凭据、阈值和通知配置的数据契约。
