# Thingsboard Server Common Protobuf and gRPC structures 模块调用链分析
...
- 测试框架入口: `common/proto/src/test/java/org/thingsboard/server/common/adaptor/JsonConverterTest.java`
- 测试框架入口: `common/proto/src/test/java/org/thingsboard/server/common/util/ProtoUtilsTest.java`


## 静态关键词统计（不等同于实际发送/写入）

- database: 46 处静态触点；未发现直接源码证据；若发生，通常在依赖的下游模块中完成。
- actor: 0 处静态触点；未发现直接 Actor API 调用，但可能通过服务端消息模型间接进入 Actor。
- mqtt: 8 处静态触点；未发现直接源码证据；若发生，通常在依赖的下游模块中完成。
- kafka: 0 处静态触点；未发现直接源码证据；若发生，通常在依赖的下游模块中完成。
- rule_engine: 2 处静态触点；未发现直接 Rule Engine 写入，但消息可能在下游规则链中继续处理。
- cache: 0 处静态触点；未发现直接源码证据；若发生，通常在依赖的下游模块中完成。
- queue: 3 处静态触点；直接操作证据：发现静态关键词触点。关键词触点 3 处仅作为辅助线索。
- rest: 0 处静态触点；未发现直接源码证据；若发生，通常在依赖的下游模块中完成。
- websocket: 19 处静态触点；直接操作证据：发现静态关键词触点。关键词触点 19 处仅作为辅助线索。
- transport: 199 处静态触点；直接操作证据：发现静态关键词触点。关键词触点 199 处仅作为辅助线索。

## 调用前后数据流

1. 调用前：上游模块已经处在具体业务流程中，需要复用稳定的公共契约或 DTO
2. 模块入口：`proto` 通过入口类、依赖 API、构建聚合或子模块暴露能力。
...
- `AdaptorException` (class, `common/proto/src/main/java/org/thingsboard/server/common/adaptor/AdaptorException.java`)
- `JsonConverter` (class, `common/proto/src/main/java/org/thingsboard/server/common/adaptor/JsonConverter.java`)
- `JsonConverterConfig` (class, `common/proto/src/main/java/org/thingsboard/server/common/adaptor/JsonConverterConfig.java`)
- `ProtoConverter` (class, `common/proto/src/main/java/org/thingsboard/server/common/adaptor/ProtoConverter.java`)
- `ProtoUtils` (class, `common/proto/src/main/java/org/thingsboard/server/common/util/ProtoUtils.java`)
- `JsonConverterTest` (class, `common/proto/src/test/java/org/thingsboard/server/common/adaptor/JsonConverterTest.java`)
- `ProtoUtilsTest` (class, `common/proto/src/test/java/org/thingsboard/server/common/util/ProtoUtilsTest.java`)
