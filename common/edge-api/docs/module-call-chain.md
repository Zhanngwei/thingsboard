# Thingsboard Server Remote Edge wrapper 模块调用链分析
...
- Spring Service 业务服务入口: `common/edge-api/src/main/java/org/thingsboard/edge/rpc/EdgeGrpcClient.java`
...
- websocket: 3 处静态触点；直接操作证据：发现静态关键词触点。关键词触点 3 处仅作为辅助线索。
- transport: 0 处静态触点；未发现直接源码证据；若发生，通常在依赖的下游模块中完成。

## 调用前后数据流

1. 调用前：上游模块已经处在具体业务流程中，需要复用稳定的公共契约或 DTO
2. 模块入口：`edge-api` 通过入口类、依赖 API、构建聚合或子模块暴露能力。
...
- `EdgeConnectionException` (class, `common/edge-api/src/main/java/org/thingsboard/edge/exception/EdgeConnectionException.java`)
- `EdgeGrpcClient` (class, `common/edge-api/src/main/java/org/thingsboard/edge/rpc/EdgeGrpcClient.java`)
- `EdgeRpcClient` (interface, `common/edge-api/src/main/java/org/thingsboard/edge/rpc/EdgeRpcClient.java`)
