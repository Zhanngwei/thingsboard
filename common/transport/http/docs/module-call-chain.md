# Thingsboard HTTP Transport Common 模块调用链分析
...
- HTTP/Web REST 控制器入口: `common/transport/http/src/main/java/org/thingsboard/server/transport/http/DeviceApiController.java`
- HTTP/Web REST 控制器入口: `common/transport/http/src/main/java/org/thingsboard/server/transport/http/DeviceApiController.java`
- HTTP/Web REST 控制器入口: `common/transport/http/src/main/java/org/thingsboard/server/transport/http/DeviceApiController.java`
- HTTP/Web REST 控制器入口: `common/transport/http/src/main/java/org/thingsboard/server/transport/http/DeviceApiController.java`
- HTTP/Web REST 控制器入口: `common/transport/http/src/main/java/org/thingsboard/server/transport/http/DeviceApiController.java`
- HTTP/Web REST 控制器入口: `common/transport/http/src/main/java/org/thingsboard/server/transport/http/DeviceApiController.java`
- HTTP/Web REST 控制器入口: `common/transport/http/src/main/java/org/thingsboard/server/transport/http/DeviceApiController.java`
- Spring Component 组件入口: `common/transport/http/src/main/java/org/thingsboard/server/transport/http/HttpTransportContext.java`
...
- mqtt: 28 处静态触点；未发现直接源码证据；若发生，通常在依赖的下游模块中完成。
...
- queue: 21 处静态触点；直接操作证据：发现静态关键词触点。关键词触点 21 处仅作为辅助线索。
- rest: 455 处静态触点；直接操作证据：发现静态关键词触点。关键词触点 455 处仅作为辅助线索。
- websocket: 609 处静态触点；直接操作证据：发现静态关键词触点。关键词触点 609 处仅作为辅助线索。
- transport: 1775 处静态触点；直接操作证据：发现静态关键词触点。关键词触点 1775 处仅作为辅助线索。

## 调用前后数据流

1. 调用前：客户端已经建立 TCP/HTTP/UDP/DTLS 等连接并携带设备凭据、主题、payload 或 RPC 响应
2. 模块入口：`http` 通过入口类、依赖 API、构建聚合或子模块暴露能力。
...
- `DeviceApiController` (class, `common/transport/http/src/main/java/org/thingsboard/server/transport/http/DeviceApiController.java`)
- `DeviceAuthCallback` (class, `common/transport/http/src/main/java/org/thingsboard/server/transport/http/DeviceApiController.java`)
- `DeviceProvisionCallback` (class, `common/transport/http/src/main/java/org/thingsboard/server/transport/http/DeviceApiController.java`)
- `GetOtaPackageCallback` (class, `common/transport/http/src/main/java/org/thingsboard/server/transport/http/DeviceApiController.java`)
- `SessionCloseOnErrorCallback` (class, `common/transport/http/src/main/java/org/thingsboard/server/transport/http/DeviceApiController.java`)
- `HttpOkCallback` (class, `common/transport/http/src/main/java/org/thingsboard/server/transport/http/DeviceApiController.java`)
- `HttpSessionListener` (class, `common/transport/http/src/main/java/org/thingsboard/server/transport/http/DeviceApiController.java`)
- `HttpTransportContext` (class, `common/transport/http/src/main/java/org/thingsboard/server/transport/http/HttpTransportContext.java`)
