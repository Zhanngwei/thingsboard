...
/**
 * 中文说明：
 * 1. 类目的：`ActivityState` 是ThingsBoard Common 模块中的传输协议契约或适配类型，用于抽象 MQTT、HTTP、CoAP、LwM2M、SNMP 与 ThingsBoard 核心消息之间的协议边界。
...
public class ActivityState<Metadata> {
    ...
    /**
     * 字段说明：
     * 1. 保存 `lastRecordedTime` 对应的配置、依赖、上下文或运行期状态。
...
/*
 * 本类总结：
 * 1. 核心职责：`ActivityState` 在 ThingsBoard Common 模块 中承担传输协议契约或适配类型职责，核心目的是抽象 MQTT、HTTP、CoAP、LwM2M、SNMP 与 ThingsBoard 核心消息之间的协议边界。
