...
/**
 * 中文说明：
 * 1. 类目的：`ThingsBoardExecutors` 是ThingsBoard Common 模块中的公共工具类型，用于提供跨模块复用的纯函数、解析、转换或辅助逻辑。
...
public class ThingsBoardExecutors {
    ...
    /**
     * 方法说明：
     * 1. 职责：执行 `newWorkStealingPool` 对应的公共工具类型流程，完成参数校验、状态读取、消息路由或结果转换。
...
/*
 * 本类总结：
 * 1. 核心职责：`ThingsBoardExecutors` 在 ThingsBoard Common 模块 中承担公共工具类型职责，核心目的是提供跨模块复用的纯函数、解析、转换或辅助逻辑。
