# Thingsboard CoAP server 模块调用链分析
...
- Spring Component 组件入口: `common/coap-server/src/main/java/org/thingsboard/server/coapserver/CoapServerContext.java`
- Spring Component 组件入口: `common/coap-server/src/main/java/org/thingsboard/server/coapserver/DefaultCoapServerService.java`
- Spring Component 组件入口: `common/coap-server/src/main/java/org/thingsboard/server/coapserver/TbCoapDtlsSettings.java`
...
- queue: 2 处静态触点；直接操作证据：发现静态关键词触点。关键词触点 2 处仅作为辅助线索。
- rest: 0 处静态触点；未发现直接源码证据；若发生，通常在依赖的下游模块中完成。
- websocket: 75 处静态触点；直接操作证据：发现静态关键词触点。关键词触点 75 处仅作为辅助线索。
- transport: 58 处静态触点；直接操作证据：发现静态关键词触点。关键词触点 58 处仅作为辅助线索。

## 调用前后数据流

1. 调用前：上游模块已经处在具体业务流程中，需要复用稳定的公共契约或 DTO
2. 模块入口：`coap-server` 通过入口类、依赖 API、构建聚合或子模块暴露能力。
...
- `CoapServerContext` (class, `common/coap-server/src/main/java/org/thingsboard/server/coapserver/CoapServerContext.java`)
- `CoapServerService` (interface, `common/coap-server/src/main/java/org/thingsboard/server/coapserver/CoapServerService.java`)
- `DefaultCoapServerService` (class, `common/coap-server/src/main/java/org/thingsboard/server/coapserver/DefaultCoapServerService.java`)
- `TbCoapDtlsCertificateVerifier` (class, `common/coap-server/src/main/java/org/thingsboard/server/coapserver/TbCoapDtlsCertificateVerifier.java`)
- `TbCoapDtlsSessionInMemoryStorage` (class, `common/coap-server/src/main/java/org/thingsboard/server/coapserver/TbCoapDtlsSessionInMemoryStorage.java`)
- `TbCoapDtlsSessionInfo` (class, `common/coap-server/src/main/java/org/thingsboard/server/coapserver/TbCoapDtlsSessionInfo.java`)
- `TbCoapDtlsSettings` (class, `common/coap-server/src/main/java/org/thingsboard/server/coapserver/TbCoapDtlsSettings.java`)
- `TbCoapServerComponent` (interface, `common/coap-server/src/main/java/org/thingsboard/server/coapserver/TbCoapServerComponent.java`)
- `TbCoapServerMessageDeliverer` (class, `common/coap-server/src/main/java/org/thingsboard/server/coapserver/TbCoapServerMessageDeliverer.java`)
