...
/**
 * 中文说明：
 * 1. 类目的：`EdgeEvent` 是ThingsBoard Common 模块中的公共数据模型类型，用于承载 ThingsBoard 实体、配置、查询、告警、通知、安全或设备画像等跨层数据契约。
...
public class EdgeEvent extends BaseData<EdgeEventId> {
    ...
    /**
     * 方法说明：
     * 1. 职责：执行 `EdgeEvent` 对应的公共数据模型类型流程，完成参数校验、状态读取、消息路由或结果转换。
...
/*
 * 本类总结：
 * 1. 核心职责：`EdgeEvent` 在 ThingsBoard Common 模块 中承担公共数据模型类型职责，核心目的是承载 ThingsBoard 实体、配置、查询、告警、通知、安全或设备画像等跨层数据契约。
