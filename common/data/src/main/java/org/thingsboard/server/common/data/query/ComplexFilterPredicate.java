...
/**
 * 中文说明：
 * 1. `ComplexFilterPredicate` 是 ThingsBoard Common Data 中承载 `Complex Filter Predicate` 信息的数据类型。
 * 2. 它把一次调用、消息传递或序列化所需的数据组织为明确结构。
 * 3. 字段分别表示该业务对象的标识、状态、内容或处理参数。
 * 4. 直接依赖的类型边界包括 `KeyFilterPredicate`。
...
public class ComplexFilterPredicate implements KeyFilterPredicate {
    ...
    /**
     * 中文说明：
     * 1. `ComplexOperation` 是 ThingsBoard Common Data 中定义 `Complex Operation` 固定取值的枚举类型。
