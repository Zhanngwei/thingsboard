...
/**
 * 中文说明：
 * 1. 类目的：`QueueRoutingInfo` 是ThingsBoard Common 模块中的公共基础设施类型，用于定义跨服务端模块复用的数据结构、接口契约或协议适配逻辑。
...
public class QueueRoutingInfo {
    ...
    /**
     * 功能：创建 `QueueRoutingInfo` 实例，并初始化必要字段。
     * 参数：
     * - `queue`：队列名称或队列对象。
     * 返回：新创建的对象实例。
     */
...
/*
 * 本类总结：
 * 1. 核心职责：`QueueRoutingInfo` 在 ThingsBoard Common 模块 中承担公共基础设施类型职责，核心目的是定义跨服务端模块复用的数据结构、接口契约或协议适配逻辑。
