...
/**
 * 中文说明：
 * 1. `AwsSqsTbVersionControlQueueFactory` 是 ThingsBoard Common Queue 中创建或提供队列对象的构造组件。
...
public class AwsSqsTbVersionControlQueueFactory implements TbVersionControlQueueFactory {
    ...
    /**
     * 功能：创建 `AwsSqsTbVersionControlQueueFactory` 实例，并初始化必要字段。
     * 参数：
     * - `sqsSettings`：配置对象。
     * - `coreSettings`：配置对象。
     * - `vcSettings`：配置对象。
     * - `sqsQueueAttributes`：队列名称或队列对象。
     * - 其余参数：补充处理条件。
     * 返回：新创建的对象实例。
     */
