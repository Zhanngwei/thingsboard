...
/**
 * 中文说明：
 * 1. `RuleEngineFlowSqlIntegrationTest` 是 ThingsBoard Application 中验证 `RuleEngineFlowSqlIntegration` 相关行为的测试类型。
 * 2. 它通过测试夹具构造输入，并执行被测类型的关键入口。
 * 3. 测试方法用准备数据、执行步骤和预期结果描述需要保持的行为。
 * 4. 直接依赖的类型边界包括 `AbstractRuleEngineFlowIntegrationTest`。
