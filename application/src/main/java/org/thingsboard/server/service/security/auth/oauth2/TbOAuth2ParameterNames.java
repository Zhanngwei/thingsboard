...
/**
 * 中文说明：
 * 1. 类目的：`TbOAuth2ParameterNames` 是ThingsBoard Application 模块中的安全认证服务类型，用于处理认证、授权、JWT、OAuth2、2FA 或会话安全流程。
...
public interface TbOAuth2ParameterNames {
    ...
    /**
     * 字段说明：
     * 1. 保存 `CALLBACK_URL_SCHEME` 对应的配置、依赖、上下文或运行期状态。
...
/*
 * 本类总结：
 * 1. 核心职责：`TbOAuth2ParameterNames` 在 ThingsBoard Application 模块 中承担安全认证服务类型职责，核心目的是处理认证、授权、JWT、OAuth2、2FA 或会话安全流程。
