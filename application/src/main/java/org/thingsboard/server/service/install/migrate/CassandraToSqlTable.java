...
/**
 * 中文说明：
 * 1. `CassandraToSqlTable` 是 ThingsBoard Application 中承载 `Cassandra To Sql` 信息的数据类型。
...
public class CassandraToSqlTable {
    ...
    /**
     * `cassandraCf` 字段，保存当前对象的对应属性。
     */
    private String cassandraCf;
    private String sqlTableName;

    /**
     * `columns`列表，用于保存一组待处理对象。
     */
    ...
    /**
     * 功能：创建 `CassandraToSqlTable` 实例，并初始化必要字段。
     * 参数：
     * - `tableName`：名称。
     * - `columns`：`columns` 参数。
     * 返回：新创建的对象实例。
    ...
    /**
     * 功能：处理`on Constraint Violation`。
     * 参数：
     * - `batchData`：待处理数据。
     * - `data`：待处理数据。
     * - `constraint`：`constraint` 参数。
     * 返回：判断结果。
     */
    ...
    /**
     * 功能：执行 `extractConstraintName` 对应的处理。
     * 参数：
     * - `ex`：`ex` 参数。
     * 返回：可能存在的结果。
     */
