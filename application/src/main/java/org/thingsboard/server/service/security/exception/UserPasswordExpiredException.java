...
/**
 * 中文说明：
 * 1. 类目的：`UserPasswordExpiredException` 是ThingsBoard Application 模块中的异常与错误响应类型，用于统一表达 ThingsBoard Application 的异常状态和 HTTP 错误响应。
...
public class UserPasswordExpiredException extends CredentialsExpiredException {
    ...
    /**
     * 方法说明：
     * 1. 职责：执行 `UserPasswordExpiredException` 对应的异常与错误响应类型流程，完成参数校验、状态读取、消息路由或结果转换。
...
/*
 * 本类总结：
 * 1. 核心职责：`UserPasswordExpiredException` 在 ThingsBoard Application 模块 中承担异常与错误响应类型职责，核心目的是统一表达 ThingsBoard Application 的异常状态和 HTTP 错误响应。
