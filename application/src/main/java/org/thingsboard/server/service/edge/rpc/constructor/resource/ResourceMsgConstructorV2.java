...
/**
 * 中文说明：
 * 1. 类目的：`ResourceMsgConstructorV2` 是ThingsBoard Application 模块中的Edge 同步服务类型，用于处理云端与边缘端之间的实体、事件和 RPC 数据同步。
...
/*
 * 本类总结：
 * 1. 核心职责：`ResourceMsgConstructorV2` 在 ThingsBoard Application 模块 中承担Edge 同步服务类型职责，核心目的是处理云端与边缘端之间的实体、事件和 RPC 数据同步。
