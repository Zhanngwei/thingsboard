...
/**
 * 中文说明：
 * 1. 类目的：`TbDashboardService` 是ThingsBoard Application 模块中的业务服务类型，用于承载 ThingsBoard 服务端应用的业务编排、实体访问和异步处理。
...
public interface TbDashboardService extends SimpleTbEntityService<Dashboard> {
    ...
    /**
     * 功能：执行 `asignDashboardToEdge` 对应的处理。
     * 参数：
     * - `tenantId`：租户IDID。
     * - `dashboardId`：仪表盘IDID。
     * - `edge`：`edge` 参数。
...
/*
 * 本类总结：
 * 1. 核心职责：`TbDashboardService` 在 ThingsBoard Application 模块 中承担业务服务类型职责，核心目的是承载 ThingsBoard 服务端应用的业务编排、实体访问和异步处理。
