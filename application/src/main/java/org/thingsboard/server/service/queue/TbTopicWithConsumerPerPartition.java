...
/**
 * 中文说明：
 * 1. 类目的：`TbTopicWithConsumerPerPartition` 是ThingsBoard Application 模块中的队列服务类型，用于封装 ThingsBoard 队列生产、消费、确认和分区处理。
...
/*
 * 本类总结：
 * 1. 核心职责：`TbTopicWithConsumerPerPartition` 在 ThingsBoard Application 模块 中承担队列服务类型职责，核心目的是封装 ThingsBoard 队列生产、消费、确认和分区处理。
