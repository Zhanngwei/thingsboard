...
/**
 * 中文说明：
 * 1. `DefaultTelemetrySubscriptionService` 是 ThingsBoard Application 中负责遥测数据的业务服务。
 * 2. 它集中组织该领域的核心操作，并向上层提供稳定的调用入口。
 * 3. 类中的依赖和状态用于完成校验、编排、查询或更新等直接职责。
 * 4. 直接依赖的类型边界包括 `AbstractSubscriptionService`、`TelemetrySubscriptionService`。
...
public class DefaultTelemetrySubscriptionService extends AbstractSubscriptionService implements TelemetrySubscriptionService {
    ...
    /**
     * 中文说明：
     * 1. `VoidFutureCallback` 是 ThingsBoard Application 中处理 `Void Future Callback` 的处理器。
