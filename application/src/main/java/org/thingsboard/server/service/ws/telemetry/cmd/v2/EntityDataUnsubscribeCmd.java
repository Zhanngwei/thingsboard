...
/**
 * 中文说明：
 * 1. 类目的：`EntityDataUnsubscribeCmd` 是ThingsBoard Application 模块中的WebSocket 服务类型，用于维护仪表盘、遥测、属性或告警订阅的 WebSocket 会话。
...
/*
 * 本类总结：
 * 1. 核心职责：`EntityDataUnsubscribeCmd` 在 ThingsBoard Application 模块 中承担WebSocket 服务类型职责，核心目的是维护仪表盘、遥测、属性或告警订阅的 WebSocket 会话。
