...
/**
 * 中文说明：
 * 1. 类目的：`DataCmd` 是ThingsBoard Application 模块中的WebSocket 服务类型，用于维护仪表盘、遥测、属性或告警订阅的 WebSocket 会话。
...
public abstract class DataCmd implements WsCmd {
    ...
    /**
     * 方法说明：
     * 1. 职责：执行 `DataCmd` 对应的WebSocket 服务类型流程，完成参数校验、状态读取、消息路由或结果转换。
...
/*
 * 本类总结：
 * 1. 核心职责：`DataCmd` 在 ThingsBoard Application 模块 中承担WebSocket 服务类型职责，核心目的是维护仪表盘、遥测、属性或告警订阅的 WebSocket 会话。
