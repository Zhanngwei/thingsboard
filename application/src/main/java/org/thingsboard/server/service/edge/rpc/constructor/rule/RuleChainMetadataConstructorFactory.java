...
/**
 * 中文说明：
 * 1. 类目的：`RuleChainMetadataConstructorFactory` 是ThingsBoard Application 模块中的Edge 同步服务类型，用于处理云端与边缘端之间的实体、事件和 RPC 数据同步。
...
public final class RuleChainMetadataConstructorFactory {
    ...
    /**
     * 方法说明：
     * 1. 职责：执行 `getByEdgeVersion` 对应的Edge 同步服务类型流程，完成参数校验、状态读取、消息路由或结果转换。
...
/*
 * 本类总结：
 * 1. 核心职责：`RuleChainMetadataConstructorFactory` 在 ThingsBoard Application 模块 中承担Edge 同步服务类型职责，核心目的是处理云端与边缘端之间的实体、事件和 RPC 数据同步。
