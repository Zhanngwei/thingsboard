...
/**
 * 中文说明：
 * 1. 类目的：`AccessJwtToken` 是ThingsBoard Application 模块中的安全认证服务类型，用于处理认证、授权、JWT、OAuth2、2FA 或会话安全流程。
...
public final class AccessJwtToken implements JwtToken {
    /**
     * 字段说明：
     * 1. 保存 `rawToken` 对应的配置、依赖、上下文或运行期状态。
    ...
    /**
     * 方法说明：
     * 1. 职责：执行 `AccessJwtToken` 对应的安全认证服务类型流程，完成参数校验、状态读取、消息路由或结果转换。
...
/*
 * 本类总结：
 * 1. 核心职责：`AccessJwtToken` 在 ThingsBoard Application 模块 中承担安全认证服务类型职责，核心目的是处理认证、授权、JWT、OAuth2、2FA 或会话安全流程。
