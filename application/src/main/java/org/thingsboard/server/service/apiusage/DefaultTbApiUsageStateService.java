...
/**
 * 中文说明：
 * 1. `DefaultTbApiUsageStateService` 是 ThingsBoard Application 中负责用量统计的业务服务。
 * 2. 它集中组织该领域的核心操作，并向上层提供稳定的调用入口。
 * 3. 类中的依赖和状态用于完成校验、编排、查询或更新等直接职责。
 * 4. 直接依赖的类型边界包括 `AbstractPartitionBasedService`、`TbApiUsageStateService`。
...
public class DefaultTbApiUsageStateService extends AbstractPartitionBasedService<EntityId> implements TbApiUsageStateService {
    ...
    /**
     * 中文说明：
     * 1. `StateChecker` 是 ThingsBoard Application 中定义 `State Checker` 能力边界的接口。
