...
/**
 * 中文说明：
 * 1. 类目的：`ThingsboardCredentialsExpiredResponse` 是ThingsBoard Application 模块中的异常与错误响应类型，用于统一表达 ThingsBoard Application 的异常状态和 HTTP 错误响应。
...
public class ThingsboardCredentialsExpiredResponse extends ThingsboardErrorResponse {
    ...
    /**
     * 方法说明：
     * 1. 职责：执行 `ThingsboardCredentialsExpiredResponse` 对应的异常与错误响应类型流程，完成参数校验、状态读取、消息路由或结果转换。
    ...
    /**
     * 方法说明：
     * 1. 职责：执行 `of` 对应的异常与错误响应类型流程，完成参数校验、状态读取、消息路由或结果转换。
     * 2. 参数：输入参数由调用方提供，通常代表请求 DTO、实体标识、租户/用户上下文、队列消息、Actor 消息或测试数据。
     * 3. 返回值：返回处理结果、响应 DTO、异步句柄或状态对象；`void` 方法通常通过副作用、回调或异常表达结果。
     * 4. 调用时机：在请求失败、认证失败或参数校验失败时创建并返回时由 Controller、Service、Actor、队列消费者、Transport 处理器或测试框架调用。
     * 5. 使用流程：捕获异常后映射错误码、状态码和响应体。
    ...
    /**
     * 方法说明：
     * 1. 职责：执行 `getResetToken` 对应的异常与错误响应类型流程，完成参数校验、状态读取、消息路由或结果转换。
     * 2. 参数：输入参数由调用方提供，通常代表请求 DTO、实体标识、租户/用户上下文、队列消息、Actor 消息或测试数据。
     * 3. 返回值：返回处理结果、响应 DTO、异步句柄或状态对象；`void` 方法通常通过副作用、回调或异常表达结果。
     * 4. 调用时机：在请求失败、认证失败或参数校验失败时创建并返回时由 Controller、Service、Actor、队列消费者、Transport 处理器或测试框架调用。
     * 5. 使用流程：捕获异常后映射错误码、状态码和响应体。
...
/*
 * 本类总结：
 * 1. 核心职责：`ThingsboardCredentialsExpiredResponse` 在 ThingsBoard Application 模块 中承担异常与错误响应类型职责，核心目的是统一表达 ThingsBoard Application 的异常状态和 HTTP 错误响应。
