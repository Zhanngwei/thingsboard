...
/**
 * 中文说明：
 * 1. `CustomOAuth2AuthorizationRequestResolver` 是 ThingsBoard Application 中处理请求的处理器。
 * 2. 它把单一处理步骤封装为可调用、可替换的组件。
 * 3. 输入通常来自上游事件、网络消息或异步回调，输出交给下一处理步骤。
 * 4. 直接依赖的类型边界包括 `OAuth2AuthorizationRequestResolver`。
...
public class CustomOAuth2AuthorizationRequestResolver implements OAuth2AuthorizationRequestResolver {
     ...
     * 参数：
     * - `request`：请求对象。
     * - `defaultAction`：`defaultAction` 参数。
     * 返回：文本结果。
     */
     ...
     * - `request`：请求对象。
     * - `registrationId`：`registrationId`ID。
     * - `redirectUriAction`：`redirectUriAction` 参数。
    ...
    /**
     * 功能：执行 `resolveRegistrationId` 对应的处理。
     * 参数：
     * - `request`：请求对象。
     * 返回：文本结果。
     */
     ...
     */
    /**
     * 功能：执行 `expandRedirectUri` 对应的处理。
     * 参数：
     * - `request`：请求对象。
     * - `clientRegistration`：客户端对象。
     * - `action`：`action` 参数。
     * 返回：文本结果。
     */
