...
/**
 * 中文说明：
 * 1. `EntityRelationController` 是 ThingsBoard Application 中处理实体关系请求的 API 控制器。
...
public class EntityRelationController extends BaseController {
    ...
    /**
     * 功能：执行 `filterRelationsByReadPermission` 对应的处理。
     * 参数：
     * - `relationsByQuery`：数据列表。
     * 返回：匹配的数据集合。
     */
