# ThingsBoard Server UI 模块调用链分析

> 生成范围：`ui-ngx`  
> Maven artifact：`ui-ngx`  
> packaging：`jar`  
...
```mermaid
flowchart TD
    A["① 最先调用<br/>浏览器用户、Angular 构建链和 Maven 前端插件最先调用 ui-ngx"]
    B["② 调用原因<br/>需要提供 ThingsBoard Web UI、静态资源和前端路由"]
    C["③ 调用之前<br/>后端 REST API 已可用，前端资源已由 npm/yarn 构建或开发服务器加载"]
    D["模块入口<br/>ui-ngx"]
    E["⑤ 数据变化<br/>表单、路由、JWT、实体 DTO 和 WebSocket 数据在浏览器状态与 REST/WebSocket payload 间转换"]
    F{"技术触点判定"}
    G["⑥ 数据库<br/>DB 间接/否"]
    ...
    L["④ 调用之后<br/>用户操作变成 HTTP/WebSocket 请求进入 application，响应更新页面状态"]
    A --> B --> C --> D --> E --> F
    F --> G
    F --> H
    F --> I
...
| 问题 | 模块级结论 |
| --- | --- |
| ① 谁最先调用这里？ | 浏览器用户、Angular 构建链和 Maven 前端插件最先调用 ui-ngx |
| ② 为什么会调用？ | 需要提供 ThingsBoard Web UI、静态资源和前端路由 |
| ③ 调用之前发生了什么？ | 后端 REST API 已可用，前端资源已由 npm/yarn 构建或开发服务器加载 |
| ④ 调用之后发生什么？ | 用户操作变成 HTTP/WebSocket 请求进入 application，响应更新页面状态 |
| ⑤ 数据如何变化？ | 表单、路由、JWT、实体 DTO 和 WebSocket 数据在浏览器状态与 REST/WebSocket payload 间转换 |
...
1. 调用前：后端 REST API 已可用，前端资源已由 npm/yarn 构建或开发服务器加载
2. 模块入口：`ui-ngx` 通过入口类、依赖 API、构建聚合或子模块暴露能力。
3. 模块内转换：表单、路由、JWT、实体 DTO 和 WebSocket 数据在浏览器状态与 REST/WebSocket payload 间转换
4. 调用后：用户操作变成 HTTP/WebSocket 请求进入 application，响应更新页面状态
