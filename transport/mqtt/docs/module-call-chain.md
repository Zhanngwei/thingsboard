# Thingsboard MQTT Transport Service 模块调用链分析
...
- Spring Boot 应用启动入口: `transport/mqtt/src/main/java/org/thingsboard/server/mqtt/ThingsboardMqttTransportApplication.java`
- 命令行或进程启动入口: `transport/mqtt/src/main/java/org/thingsboard/server/mqtt/ThingsboardMqttTransportApplication.java`
...
- mqtt: 14 处静态触点；未发现直接源码证据；若发生，通常在依赖的下游模块中完成。
...
- `ThingsboardMqttTransportApplication` (class, `transport/mqtt/src/main/java/org/thingsboard/server/mqtt/ThingsboardMqttTransportApplication.java`)
