...
/**
 * 中文说明：
 * 1. 类目的：`ThingsboardSnmpTransportApplication` 是 ThingsBoard SNMP Transport 启动模块 中的SNMP 独立协议服务启动入口，用于启动独立的 ThingsBoard SNMP transport Spring Boot 进程，并加载协议包、common、queue 和 cache 相关组件。
 * 2. 所属模块：位于 transport/snmp，服务于 ThingsBoard 的客户端访问、离线工具或独立协议接入边界。
 * 3. 协作模块：主要协作对象包括 SNMP transport 实现、ThingsBoard common transport、queue、cache、Spring Boot 自动配置和部署脚本。
...
public class ThingsboardSnmpTransportApplication {
     ...
     * 3. 生命周期：字段生命周期与 `SNMP 独立协议服务启动入口` 实例或类加载周期一致；静态常量随类加载存在，实例状态随单次客户端会话、迁移命令或 Spring Boot 进程存在。
     ...
     * 6. 事务/缓存/MQTT/Actor/数据库/Rule Engine：字段本身不打开事务；是否涉及缓存、MQTT、Actor、数据库或规则链取决于 ThingsBoard SNMP Transport 启动模块 的上层流程。
     */
    ...
    /**
     * 方法说明：
     * 1. 职责：`main` 执行 SNMP 独立协议服务启动入口 的一个明确步骤，完成请求发送、参数转换、文件解析、writer 构造、TLS/MQTT 操作或 Spring Boot 启动参数处理。
    ...
    /**
     * 方法说明：
     * 1. 职责：`updateArguments` 执行 SNMP 独立协议服务启动入口 的一个明确步骤，完成请求发送、参数转换、文件解析、writer 构造、TLS/MQTT 操作或 Spring Boot 启动参数处理。
     * 2. 输入参数：参数通常表示 REST DTO/ID、分页条件、文件路径、dump 行、Cassandra 行值、命令行参数、证书配置、MQTT 消息或 Spring Boot 启动参数。
     * 3. 返回值：返回服务端 DTO、转换后的 KV/行值、writer、更新后的参数数组、Optional/PageData/byte[]，或通过 `void` 的副作用完成发送、写入、启动、关闭和断言式失败。
     * 4. 调用时机：由操作系统服务、Docker 容器或命令行启动，SpringApplication 创建上下文后持续运行直到进程关闭；由 SDK 调用方、命令行入口、迁移编排器、SpringApplication 或类内辅助流程触发。
     * 5. 调用方：可能是外部 Java 客户端、测试/运维脚本、MigratorTool、PgCaMigrator、Spring Boot launcher 或本类其它辅助方法。
    ...
    /**
     * 本类总结：
     * 1. 核心职责：`ThingsboardSnmpTransportApplication` 负责启动独立的 ThingsBoard SNMP transport Spring Boot 进程，并加载协议包、common、queue 和 cache 相关组件。
     * 2. 核心流程：main 方法接收命令行参数，补齐默认 spring.config.name 后启动 Spring Boot，上下文再加载协议 handler、队列生产者、缓存和调度任务。
     * 3. 关键依赖：SNMP transport 实现、ThingsBoard common transport、queue、cache、Spring Boot 自动配置和部署脚本。
