# 第 09 章：LwM2M Transport

## 本章定位

**学习目标：** 能分析 LwM2M 注册、Bootstrap、Observe、Read/Write/Execute、OTA 和 RPC 映射。

**为什么学习：** LwM2M 是设备管理协议，覆盖设备生命周期、对象模型、安全和固件升级，是高级 IoT 项目的关键能力。

**与下一章节关系：** 下一章学习 SNMP，理解平台主动轮询型设备接入。
...
```mermaid
flowchart TD
    A["设备/网关"] --> B["协议报文入口<br/>LwM2M Transport"]
    B --> C["解析身份、路径、主题或对象"]
...
```text
LwM2M Register:
POST coap://server/rd?ep=device-001&lt=3600&b=U
Payload: </1/0>,</3/0>,</3303/0>

Observe temperature:
GET coap://device/3303/0/5700
Option: Observe=0
```
...
- `coap && (udp.port == 5683 || udp.port == 5684)`：LwM2M 基于 CoAP/DTLS。
- `coap.opt.uri_path == "rd"`：Registration。
- `coap.opt.uri_path == "bs"`：Bootstrap。
- `coap.opt.observe == 0`：Observe 注册。
- `coap.code == 0.02 || coap.code == 0.03 || coap.code == 0.04`：POST/PUT/DELETE 操作。

### 抓包内容

- Bootstrap 请求与 Bootstrap Finish。
- Register `/rd?ep=...&lt=...`。
- Update Registration。
- Observe `/object/instance/resource`。
- Notify 上报资源值。
- Read/Write/Execute 下行操作。
- Firmware/Software OTA 相关对象操作。

### 报文字段逐项解释

| 层级 | 字段 | 分析意义 |
|---|---|---|
| CoAP Header | `type/code/message id/token` | LwM2M 请求响应和重传关联。 |
| Uri-Path | `rd/bs/3/0/0/3303/0/5700` | LwM2M 接口和对象资源路径。 |
| Uri-Query | `ep/lt/b/lwm2m` | Endpoint、生命周期、Binding、版本。 |
| Content-Format | `TLV/JSON/SenML CBOR/Link Format` | 对象资源编码格式。 |
| Observe | `0/sequence` | 订阅和 Notify 序号。 |
| Payload | `Object Link/Resource Value` | 设备模型、资源值或 OTA 状态。 |

### 对应源码、Netty、Handler、Actor、数据库

| 维度 | 对应位置 |
|---|---|
| 源码 | `common/transport/lwm2m/src/main/java/org/thingsboard/server/transport/lwm2m/server/DefaultLwM2mTransportService.java` |
| 源码 | `common/transport/lwm2m/src/main/java/org/thingsboard/server/transport/lwm2m/server/LwM2mServerListener.java` |
| 源码 | `common/transport/lwm2m/src/main/java/org/thingsboard/server/transport/lwm2m/server/uplink/DefaultLwM2mUplinkMsgHandler.java` |
| 源码 | `common/transport/lwm2m/src/main/java/org/thingsboard/server/transport/lwm2m/server/downlink/DefaultLwM2mDownlinkMsgHandler.java` |
| 源码 | `common/transport/lwm2m/src/main/java/org/thingsboard/server/transport/lwm2m/server/ota/DefaultLwM2MOtaUpdateService.java` |
| Netty | LwM2M 基于 Leshan/Californium，不经过 ThingsBoard MQTT Netty Handler。 |
