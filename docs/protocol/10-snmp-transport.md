# 第 10 章：SNMP Transport

## 本章定位

**学习目标：** 理解轮询型设备接入、OID 映射、Trap 和 SNMPv3 安全模型。

**为什么学习：** SNMP 代表平台主动采集设备状态的模型，适合网络设备、机房设备和部分工业设备。

**与下一章节关系：** 下一章学习 Modbus TCP，进入 PLC/仪表寄存器采集模型。
...
```mermaid
flowchart TD
    A["设备/网关"] --> B["协议报文入口<br/>SNMP Transport"]
    B --> C["解析身份、路径、主题或对象"]
...
- `snmp`：SNMP 报文。
- `udp.port == 161`：Get/Set/Walk。
- `udp.port == 162`：Trap。
- `snmp.version == 1 || snmp.version == 2 || snmp.version == 3`：按版本过滤。
- `snmp.name contains "1.3.6.1"`：按 OID 搜索。

### 抓包内容

- GET/GETNEXT/GETBULK 请求。
- Response PDU。
- Trap/Inform。
- SNMPv3 USM 安全参数。
- VarBind OID 与值。
...
| 层级 | 字段 | 分析意义 |
|---|---|---|
| SNMP Header | `version/community or msgGlobalData` | 协议版本和安全上下文。 |
| PDU | `type/request-id/error-status/error-index` | 请求类型、关联 ID 和错误状态。 |
| VarBind | `OID/value type/value` | 采集点位和返回值。 |
| SNMPv3 Security | `engineId/user/auth/priv` | 认证和加密参数。 |
| UDP | `srcport/dstport/length` | 轮询和 Trap 的网络路径。 |

### 对应源码、Netty、Handler、Actor、数据库

| 维度 | 对应位置 |
|---|---|
| 源码 | `common/transport/snmp/src/main/java/org/thingsboard/server/transport/snmp/service/SnmpTransportService.java` |
| 源码 | `common/transport/snmp/src/main/java/org/thingsboard/server/transport/snmp/service/PduService.java` |
| 源码 | `common/transport/snmp/src/main/java/org/thingsboard/server/transport/snmp/service/SnmpAuthService.java` |
| 源码 | `common/transport/snmp/src/main/java/org/thingsboard/server/transport/snmp/session/DeviceSessionContext.java` |
| Netty | SNMP Transport 使用 SNMP4J，不经过 Netty。 |
...
1. SNMP Transport 在 ThingsBoard 通信链路中解决什么问题？

**参考答案：**
SNMP Transport 在 ThingsBoard 通信链路中的作用是：SNMP Transport 解决平台主动轮询网络/工业设备 OID，并处理 Trap/Inform 事件。 面试时可以按“入口、转换、投递、处理、落库”五步回答。第一步，设备或网关先通过对应协议把数据送到服务端；第二步，入口层完成协议字段解析和身份识别；第三步，Transport 层把外部协议负载转换成 ThingsBoard 内部消息；第四步，消息通过 Queue/Core/Actor/Rule Engine 处理；第五步，最终由 DAO 写入遥测、属性、事件、告警或 RPC 状态。

**示例说法：**
“我理解的 SNMP Transport 不是一个孤立协议点，而是 ThingsBoard 设备接入链路的一段。Transport 按配置周期发送 SNMP Get/GetNext/GetBulk，设备返回 VarBind；Trap 则由设备主动发往 162 端口。 进入平台后，不能直接写业务表，而要先收敛到 Transport 抽象，再进入队列、Actor 和 Rule Engine。这样 HTTP、MQTT、CoAP、LwM2M、SNMP 或工业网关数据最终都能复用同一套设备状态、规则链和存储能力。”

2. 设备身份从报文哪个字段进入平台，最终如何映射到 DeviceId？

**参考答案：**
设备身份首先来自协议或网关映射层。设备身份来自 Transport 配置、目标 IP/端口、SNMP 凭据和 ThingsBoard 中的设备映射，而不是报文本身的 access token。 ThingsBoard 不会只凭 IP 判断设备，因为 IP 可能经过 NAT、网关或负载均衡，不能稳定代表设备。身份字段经过认证服务后，会映射成 TenantId、DeviceId、DeviceProfileId、SessionId 等内部上下文。这些字段后续会进入 Queue 消息、Actor 路由、Rule Engine 的 `TbMsg` metadata，以及 DAO 查询和写入条件。
...
**参考答案：**
是否经过 Netty 要看入口协议。SNMP 使用 SNMP4J 和 `SnmpTransportService`、`PduService`，不经过 Netty。 面试时要避免把所有协议都说成 Netty。Netty 适合 TCP 长连接和自定义二进制协议，但 ThingsBoard 中 HTTP、CoAP、LwM2M、SNMP 等入口可能分别由 Spring MVC、Californium/Leshan、SNMP4J 等框架承接。关键不是框架名字，而是找到“外部报文第一次进入业务代码”的入口类。
...
**参考答案：**
正常业务链路会进入 Actor，但不是网络包一到就直接进 Actor。采集到的 OID 值转换成遥测/属性后，经 TransportService 和队列进入 DeviceActor。 Actor 的价值是把设备、租户、规则链等有状态对象串行化处理，降低锁竞争，并保证同一设备或同一规则节点相关状态的一致性。通常路径是：Transport 解析并投递队列，Core Consumer 消费后进入 TenantActor，再路由到 DeviceActor；需要规则处理时进入 RuleChainActor 和 RuleNodeActor。
...
**参考答案：**
是否进入 Rule Engine 取决于消息类型。OID 采集值转换成遥测或属性后会进入 Rule Engine；Trap 可转成事件或遥测消息。 一般来说，遥测上报、属性上报、连接事件、部分 RPC 或生命周期事件可以进入规则链；纯协议握手、订阅请求、ACK、心跳、底层重传通常不会作为业务消息进入 Rule Engine。Rule Engine 处理的是平台标准消息，而不是原始 TCP/UDP/MQTT/CoAP 字节。
...
**参考答案：**
这条链路涉及的数据库和缓存要按阶段区分。设备配置和 SNMP 凭据读取自设备/属性/配置，采集结果写 `ts_kv` 或 `attribute_kv`，异常和事件可写 `event`。 另外，ThingsBoard 为了减少高频查询，会使用本地 Caffeine 缓存、Redis 缓存或专门的 Transport 缓存保存设备凭据、设备 Profile、租户 Profile、实体关系、属性和限流状态。回答时不要只说“写数据库”，而要说明读什么、写什么、何时写。
...
**参考答案：**
生产延迟要分层定位，不能只看 JVM CPU。优先看轮询周期、SNMP 超时、重试次数、目标设备响应时间、PDU 批量大小、Transport 负载均衡和队列积压。 一般我会按顺序看：网络 RTT 和重传、协议确认耗时、入口线程或 EventLoop、认证和缓存命中率、Transport 限流、队列发送耗时、Kafka lag、Consumer 处理速率、Actor mailbox、Rule Engine 节点耗时、DAO 慢查询、数据库连接池、WebSocket 推送延迟。这样可以把“设备慢”“平台慢”“规则慢”“数据库慢”区分开。
