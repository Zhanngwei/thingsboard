# 第 02 章：TCP、UDP、TLS、DTLS

## 本章定位

**学习目标：** 能解释可靠流、无连接报文、加密握手和 DTLS 重传对 IoT 设备在线率的影响。

**为什么学习：** MQTT/Modbus/OPC UA 依赖 TCP，CoAP/LwM2M/SNMP 常依赖 UDP，安全接入依赖 TLS/DTLS。

**与下一章节关系：** 下一章用 Netty 把这些网络概念落到 Java 服务端事件模型。
...
- TCP 三次握手和四次挥手
- 滑动窗口、MSS、MTU
- Nagle、KeepAlive、TIME_WAIT
- UDP 丢包、乱序、NAT
- TLS Record 与 Handshake
- DTLS epoch、sequence、fragment
- 证书、公钥、私钥、信任链
- 弱网下重试策略
...
```mermaid
flowchart TD
    A["设备/网关"] --> B["协议报文入口<br/>TCP、UDP、TLS、DTLS"]
    B --> C["解析身份、路径、主题或对象"]
...
```bash
# TLS 握手验证
openssl s_client -connect localhost:8883 -servername localhost -tls1_2

# UDP/DTLS 端口观察
tcpdump -i any -nn "udp port 5684"
```
...
| 维度 | 对应位置 |
|---|---|
| 源码 | `common/transport/transport-api/src/main/java/org/thingsboard/server/common/transport/config/ssl/SslCredentials.java` |
| 源码 | `common/transport/transport-api/src/main/java/org/thingsboard/server/common/transport/config/ssl/PemSslCredentials.java` |
| 源码 | `common/transport/lwm2m/src/main/java/org/thingsboard/server/transport/lwm2m/secure/TbLwM2MDtlsCertificateVerifier.java` |
...
1. TCP、UDP、TLS、DTLS 在 ThingsBoard 通信链路中解决什么问题？

**参考答案：**
TCP、UDP、TLS、DTLS 在 ThingsBoard 通信链路中的作用是：TCP/UDP/TLS/DTLS 解决可靠传输、无连接传输和安全传输问题，是 MQTT、CoAP、LwM2M、SNMP、Modbus、OPC UA 的通信基础。 面试时可以按“入口、转换、投递、处理、落库”五步回答。第一步，设备或网关先通过对应协议把数据送到服务端；第二步，入口层完成协议字段解析和身份识别；第三步，Transport 层把外部协议负载转换成 ThingsBoard 内部消息；第四步，消息通过 Queue/Core/Actor/Rule Engine 处理；第五步，最终由 DAO 写入遥测、属性、事件、告警或 RPC 状态。

**示例说法：**
“我理解的 TCP、UDP、TLS、DTLS 不是一个孤立协议点，而是 ThingsBoard 设备接入链路的一段。TCP 协议先完成三次握手，TLS 再完成证书和密钥协商；UDP 不建连接，DTLS 在 UDP 上模拟安全会话。 进入平台后，不能直接写业务表，而要先收敛到 Transport 抽象，再进入队列、Actor 和 Rule Engine。这样 HTTP、MQTT、CoAP、LwM2M、SNMP 或工业网关数据最终都能复用同一套设备状态、规则链和存储能力。”

2. 设备身份从报文哪个字段进入平台，最终如何映射到 DeviceId？

**参考答案：**
设备身份首先来自协议或网关映射层。TCP/UDP 层只提供 IP、端口和连接五元组；ThingsBoard 身份仍在上层，例如 MQTT username、HTTP token、X.509 证书或 LwM2M endpoint。 ThingsBoard 不会只凭 IP 判断设备，因为 IP 可能经过 NAT、网关或负载均衡，不能稳定代表设备。身份字段经过认证服务后，会映射成 TenantId、DeviceId、DeviceProfileId、SessionId 等内部上下文。这些字段后续会进入 Queue 消息、Actor 路由、Rule Engine 的 `TbMsg` metadata，以及 DAO 查询和写入条件。
...
**参考答案：**
是否经过 Netty 要看入口协议。MQTT/TCP 可进入 Netty Handler；CoAP/LwM2M 使用 UDP/DTLS 协议栈；SNMP 使用 UDP 和 SNMP4J；Modbus/OPC UA 通常在网关侧处理。 面试时要避免把所有协议都说成 Netty。Netty 适合 TCP 长连接和自定义二进制协议，但 ThingsBoard 中 HTTP、CoAP、LwM2M、SNMP 等入口可能分别由 Spring MVC、Californium/Leshan、SNMP4J 等框架承接。关键不是框架名字，而是找到“外部报文第一次进入业务代码”的入口类。
...
**参考答案：**
正常业务链路会进入 Actor，但不是网络包一到就直接进 Actor。TCP/UDP/TLS/DTLS 本身不会触发 Actor，只有上层 Transport 把数据转换成内部消息后才进入 Actor。 Actor 的价值是把设备、租户、规则链等有状态对象串行化处理，降低锁竞争，并保证同一设备或同一规则节点相关状态的一致性。通常路径是：Transport 解析并投递队列，Core Consumer 消费后进入 TenantActor，再路由到 DeviceActor；需要规则处理时进入 RuleChainActor 和 RuleNodeActor。
...
**参考答案：**
是否进入 Rule Engine 取决于消息类型。TLS 或 DTLS 握手成功不是业务成功；必须完成协议解析和 TransportService 投递后，遥测/属性等才会进入 Rule Engine。 一般来说，遥测上报、属性上报、连接事件、部分 RPC 或生命周期事件可以进入规则链；纯协议握手、订阅请求、ACK、心跳、底层重传通常不会作为业务消息进入 Rule Engine。Rule Engine 处理的是平台标准消息，而不是原始 TCP/UDP/MQTT/CoAP 字节。
...
**参考答案：**
这条链路涉及的数据库和缓存要按阶段区分。传输层不写数据库。上层认证与业务处理成功后才会读取凭据并写入遥测、属性、RPC 或事件表。 另外，ThingsBoard 为了减少高频查询，会使用本地 Caffeine 缓存、Redis 缓存或专门的 Transport 缓存保存设备凭据、设备 Profile、租户 Profile、实体关系、属性和限流状态。回答时不要只说“写数据库”，而要说明读什么、写什么、何时写。
...
**参考答案：**
生产延迟要分层定位，不能只看 JVM CPU。优先检查 RTT、重传、窗口大小、MTU、TLS 握手耗时、证书校验耗时、DTLS 重传和 UDP 丢包。 一般我会按顺序看：网络 RTT 和重传、协议确认耗时、入口线程或 EventLoop、认证和缓存命中率、Transport 限流、队列发送耗时、Kafka lag、Consumer 处理速率、Actor mailbox、Rule Engine 节点耗时、DAO 慢查询、数据库连接池、WebSocket 推送延迟。这样可以把“设备慢”“平台慢”“规则慢”“数据库慢”区分开。
