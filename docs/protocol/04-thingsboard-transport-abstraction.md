# 第 04 章：ThingsBoard Transport 抽象

## 本章定位

**学习目标：** 理解协议无关的 Transport 模型，能判断新协议应该复用哪些接口、扩展哪些类。

**为什么学习：** 自定义 Transport 不是复制 MQTT，而是实现统一的认证、Session、消息转换和队列投递契约。

**与下一章节关系：** 下一章用 HTTP Transport 作为最简单入口，观察抽象模型如何承接真实请求。
...
```mermaid
flowchart TD
    A["设备/网关"] --> B["协议报文入口<br/>ThingsBoard Transport 抽象"]
    B --> C["解析身份、路径、主题或对象"]
...
```java
public final class DemoTransportAdaptor implements TransportAdaptor<String> {
    @Override
    public TransportProtos.PostTelemetryMsg convertToPostTelemetry(
            DeviceSessionCtx ctx, String payload) throws AdaptorException {
...
| 维度 | 对应位置 |
|---|---|
| 源码 | `common/transport/transport-api/src/main/java/org/thingsboard/server/common/transport/TransportService.java` |
...
| 源码 | `common/transport/transport-api/src/main/java/org/thingsboard/server/common/transport/TransportAdaptor.java` |
| 源码 | `common/transport/transport-api/src/main/java/org/thingsboard/server/common/transport/session/SessionContext.java` |
...
1. ThingsBoard Transport 抽象 在 ThingsBoard 通信链路中解决什么问题？

**参考答案：**
ThingsBoard Transport 抽象 在 ThingsBoard 通信链路中的作用是：ThingsBoard Transport 抽象把 HTTP、MQTT、CoAP、LwM2M、SNMP 等外部协议统一成平台内部消息。 面试时可以按“入口、转换、投递、处理、落库”五步回答。第一步，设备或网关先通过对应协议把数据送到服务端；第二步，入口层完成协议字段解析和身份识别；第三步，Transport 层把外部协议负载转换成 ThingsBoard 内部消息；第四步，消息通过 Queue/Core/Actor/Rule Engine 处理；第五步，最终由 DAO 写入遥测、属性、事件、告警或 RPC 状态。

**示例说法：**
“我理解的 ThingsBoard Transport 抽象 不是一个孤立协议点，而是 ThingsBoard 设备接入链路的一段。不同协议入口先完成解析和认证，然后通过 `TransportAdaptor` 和 `DefaultTransportService` 进入统一链路。 进入平台后，不能直接写业务表，而要先收敛到 Transport 抽象，再进入队列、Actor 和 Rule Engine。这样 HTTP、MQTT、CoAP、LwM2M、SNMP 或工业网关数据最终都能复用同一套设备状态、规则链和存储能力。”

2. 设备身份从报文哪个字段进入平台，最终如何映射到 DeviceId？

**参考答案：**
设备身份首先来自协议或网关映射层。身份由各协议入口提取后交给 Transport 认证服务，最终映射到 TenantId、DeviceId、DeviceProfile 和 Session 信息。 ThingsBoard 不会只凭 IP 判断设备，因为 IP 可能经过 NAT、网关或负载均衡，不能稳定代表设备。身份字段经过认证服务后，会映射成 TenantId、DeviceId、DeviceProfileId、SessionId 等内部上下文。这些字段后续会进入 Queue 消息、Actor 路由、Rule Engine 的 `TbMsg` metadata，以及 DAO 查询和写入条件。
...
**参考答案：**
是否经过 Netty 要看入口协议。入口可以是 Netty Handler、Spring Controller、CoAP Resource 或协议 Service，但进入平台后都收敛到 `TransportService`。 面试时要避免把所有协议都说成 Netty。Netty 适合 TCP 长连接和自定义二进制协议，但 ThingsBoard 中 HTTP、CoAP、LwM2M、SNMP 等入口可能分别由 Spring MVC、Californium/Leshan、SNMP4J 等框架承接。关键不是框架名字，而是找到“外部报文第一次进入业务代码”的入口类。
...
**参考答案：**
正常业务链路会进入 Actor，但不是网络包一到就直接进 Actor。TransportService 投递到队列后，由 Core Consumer 消费并路由到 TenantActor 和 DeviceActor。 Actor 的价值是把设备、租户、规则链等有状态对象串行化处理，降低锁竞争，并保证同一设备或同一规则节点相关状态的一致性。通常路径是：Transport 解析并投递队列，Core Consumer 消费后进入 TenantActor，再路由到 DeviceActor；需要规则处理时进入 RuleChainActor 和 RuleNodeActor。
...
**参考答案：**
是否进入 Rule Engine 取决于消息类型。遥测、属性、RPC、连接事件等消息会根据类型进入 Rule Engine 或 Core 设备状态处理。 一般来说，遥测上报、属性上报、连接事件、部分 RPC 或生命周期事件可以进入规则链；纯协议握手、订阅请求、ACK、心跳、底层重传通常不会作为业务消息进入 Rule Engine。Rule Engine 处理的是平台标准消息，而不是原始 TCP/UDP/MQTT/CoAP 字节。
...
**参考答案：**
生产延迟要分层定位，不能只看 JVM CPU。优先检查 Transport 限流、缓存命中率、队列发送耗时、回调是否失败、Core Consumer lag 和 Actor mailbox。 一般我会按顺序看：网络 RTT 和重传、协议确认耗时、入口线程或 EventLoop、认证和缓存命中率、Transport 限流、队列发送耗时、Kafka lag、Consumer 处理速率、Actor mailbox、Rule Engine 节点耗时、DAO 慢查询、数据库连接池、WebSocket 推送延迟。这样可以把“设备慢”“平台慢”“规则慢”“数据库慢”区分开。
