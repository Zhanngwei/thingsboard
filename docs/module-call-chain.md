...
```mermaid
flowchart TD
    A["① 最先调用<br/>Maven reactor、构建脚本或开发者最先调用根聚合项目"]
    B["② 调用原因<br/>需要统一构建、测试、打包和管理所有 ThingsBoard 子模块"]
    C["③ 调用之前<br/>开发者选择 profile、JDK、数据库/队列参数和 Maven goals"]
    D["模块入口<br/>thingsboard"]
    E["⑤ 数据变化<br/>数据主要是构建元数据、依赖版本、profile 属性和模块列表，不承载运行时业务数据"]
    F{"技术触点判定"}
    G["⑥ 数据库<br/>DB 间接/否"]
    ...
    L["④ 调用之后<br/>Maven 按模块依赖顺序进入子模块，运行编译、测试、打包或 Docker/前端构建步骤"]
    A --> B --> C --> D --> E --> F
    F --> G
    F --> H
    F --> I
...
| 问题 | 模块级结论 |
| --- | --- |
| ① 谁最先调用这里？ | Maven reactor、构建脚本或开发者最先调用根聚合项目 |
| ② 为什么会调用？ | 需要统一构建、测试、打包和管理所有 ThingsBoard 子模块 |
| ③ 调用之前发生了什么？ | 开发者选择 profile、JDK、数据库/队列参数和 Maven goals |
| ④ 调用之后发生什么？ | Maven 按模块依赖顺序进入子模块，运行编译、测试、打包或 Docker/前端构建步骤 |
| ⑤ 数据如何变化？ | 数据主要是构建元数据、依赖版本、profile 属性和模块列表，不承载运行时业务数据 |
...
1. 调用前：开发者选择 profile、JDK、数据库/队列参数和 Maven goals
2. 模块入口：`thingsboard` 通过入口类、依赖 API、构建聚合或子模块暴露能力。
3. 模块内转换：数据主要是构建元数据、依赖版本、profile 属性和模块列表，不承载运行时业务数据
4. 调用后：Maven 按模块依赖顺序进入子模块，运行编译、测试、打包或 Docker/前端构建步骤
