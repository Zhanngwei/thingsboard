...
/**
 * 中文说明：
 * 1. 类目的：`AlarmCommentEntity` 是 ThingsBoard DAO 模块 中的持久化实体映射类型，用于描述 ThingsBoard 领域对象与 SQL/Cassandra 存储结构之间的字段映射、索引关系和序列化边界。
...
public class AlarmCommentEntity extends AbstractAlarmCommentEntity<AlarmComment>  {
    ...
    /**
     * 功能：创建 `AlarmCommentEntity` 实例，并初始化必要字段。
     * 参数：无。
     * 返回：新创建的对象实例。
     */
     ...
     * 功能：创建 `AlarmCommentEntity` 实例，并初始化必要字段。
     * 参数：
     * - `alarmCommentInfo`：`alarmCommentInfo` 参数。
     * 返回：新创建的对象实例。
     */
...
/*
 * 本类总结：
 * 1. 核心职责：`AlarmCommentEntity` 在 ThingsBoard DAO 模块 中承担持久化实体映射类型职责，核心目的是描述 ThingsBoard 领域对象与 SQL/Cassandra 存储结构之间的字段映射、索引关系和序列化边界。
