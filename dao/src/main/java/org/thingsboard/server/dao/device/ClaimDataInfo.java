...
/**
 * 中文说明：
 * 1. 类目的：`ClaimDataInfo` 是 ThingsBoard DAO 模块 中的设备持久化服务类型，用于维护设备、凭据、配置、声明、连接状态和设备画像相关的数据库访问流程。
...
public class ClaimDataInfo {
    ...
    /**
     * 字段说明：
     * 1. 保存 `fromCache` 对应的 DAO 依赖、Repository、缓存、配置、上下文或测试状态。
...
/*
 * 本类总结：
 * 1. 核心职责：`ClaimDataInfo` 在 ThingsBoard DAO 模块 中承担设备持久化服务类型职责，核心目的是维护设备、凭据、配置、声明、连接状态和设备画像相关的数据库访问流程。
