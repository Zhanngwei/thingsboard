...
/**
 * 中文说明：
 * 1. 类目的：`NoXssValidator` 是 ThingsBoard DAO 模块 中的DAO 服务测试或服务支撑类型，用于组织 DAO 层测试、共享服务夹具或持久化服务的公共执行流程。
...
public class NoXssValidator implements ConstraintValidator<NoXss, Object> {
    ...
    /**
     * 字段说明：
     * 1. 保存 `xssPolicy` 对应的 DAO 依赖、Repository、缓存、配置、上下文或测试状态。
...
/*
 * 本类总结：
 * 1. 核心职责：`NoXssValidator` 在 ThingsBoard DAO 模块 中承担DAO 服务测试或服务支撑类型职责，核心目的是组织 DAO 层测试、共享服务夹具或持久化服务的公共执行流程。
